document.addEventListener("DOMContentLoaded", async function() {
    ...
    // Ensure templates and data are loaded
    ...
    if (azFilterButtonsContainer) {
        ...
        azFilterButtonsContainer.addEventListener('click', (event) => {
            if (event.target.classList.contains('btn')) {
                ...
                azFilterButtonsContainer.querySelectorAll('.btn').forEach(btn => btn.classList.remove('active'));
    ...
    if (azContentGrid) {
        try {
            ...
            azContentGrid.innerHTML = '<p>Could not load content. Please try again later.</p>';
        } finally {
            if(preloader) {
        ...
        console.warn("azContentGrid element not found.");
        if (preloader) {
            preloader.classList.add('loaded');
        }
    }
