...
document.addEventListener("DOMContentLoaded", function() {
    ...
    [loginEmailInput, loginPasswordInput, signupUsernameInput, signupEmailInput, signupPasswordInput, signupConfirmPasswordInput].forEach(input => {
        if (input) input.addEventListener("input", () => clearError(input));
    });
    if (signupTermsCheckbox) signupTermsCheckbox.addEventListener("change", () => clearError(signupTermsCheckbox));
    ...
    if (loginSubmitBtn) {
        loginSubmitBtn.addEventListener("click", async (e) => {
            ...
            clearFormErrors();
            let isValid = true;
            if (!loginEmailInput.value.trim()) { displayError(loginEmailInput, "Email or Username is required."); isValid = false; }
            if (!loginPasswordInput.value.trim()) { displayError(loginPasswordInput, "Password is required."); isValid = false; }
            if (!isValid) return;
            ...
            try {
                ...
                if (!user) {
                    showNotification("User not found. Please check your details.", 'error');
                    ...
                    // --- [MODIFIED] Pass the full user object ---
                    updateAuthUI(user);
                    hideAuthModal();
    ...
    if (signupSubmitBtn) {
        signupSubmitBtn.addEventListener("click", async (e) => {
            ...
            if (!username) { displayError(signupUsernameInput, "Username is required."); isValid = false; }
            if (!email || !validateEmail(email)) { displayError(signupEmailInput, "Please enter a valid email address."); isValid = false; }
            if (!password || !validatePassword(password)) { displayError(signupPasswordInput, "Password must be at least 6 characters."); isValid = false; }
            if (password !== confirmPassword) { displayError(signupConfirmPasswordInput, "Passwords do not match."); isValid = false; }
            if (signupTermsCheckbox && !signupTermsCheckbox.checked) { displayError(signupTermsCheckbox, "You must agree to the terms."); isValid = false; }
            if (!isValid) return;
            ...
            try {
                ...
                if (!errorFound) {
                    const newUser = { username: sanitizeInput(username), email: email, password: password };
                    users.push(newUser);
                    ...
                    hideAuthModal();
                    showNotification("Signup successful! Welcome to StreamVerse!", 'success');
                    window.dispatchEvent(new CustomEvent('userLoggedIn'));
                }
            } catch (error) {
    ...
    // --- [MODIFIED] Function now accepts a full user object ---
    function updateAuthUI(user) {
        ...
        const existingUserProfileWrapper = document.getElementById("userProfileToggle")?.closest('.nav-item-dropdown-wrapper');
        ...
        // --- [MODIFIED] Added user.username to the dropdown ---
    ...
    function checkLoginStatus() {
        ...
        if (loggedInUserEmail) {
            // --- [MODIFIED] Find the full user object to pass to updateAuthUI ---
            const allUsers = JSON.parse(localStorage.getItem("streamVerseUsers") || "[]");
    ...
    if (typeof ensureTemplatesLoaded === "function") {
         ensureTemplatesLoaded().then(() => { checkLoginStatus(); initializeAuthToggle(); });
    } else {
          setTimeout(() => { checkLoginStatus(); initializeAuthToggle(); }, 750);
    }
});